...
final class DependencyPropertyParser {
	...
	public static Result parse(String key) {
		if (!key.contains("@")) {
			throw new IllegalArgumentException("Property '" + key + "' is not a properly formatted key since it does not contain '@'");
		}

		final String[] artifactAndModule = key.split("@");
		if (artifactAndModule.length != 2) {
			throw new IllegalArgumentException("Property '" + key + "' is not a properly formatted key since it is not properly split by '@'");
		}
